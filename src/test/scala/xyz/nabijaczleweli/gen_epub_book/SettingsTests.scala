...
class SettingsTests {
	@Test
	def correct() {
		val s = new Settings(Array(
			"-Irpf=examples/relative_path_fuckery/relative/path", "examples/simple.epupp", "-Isrc", "out/simple.epub", "-Iw=gradle/wrapper", "-S="))
		assert(s.inFile equals new File("examples/simple.epupp"))
		assert(s.outFile equals new File("out/simple.epub"))
