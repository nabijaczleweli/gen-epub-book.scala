...
class ElementTests {
	...
	@Test
	def `parseLine(custom separator)`() {
		assert(Element.parseLine("Name INCREDIBLE COMMUNISM Everything we got, in one thing", "INCREDIBLE COMMUNISM").contains(NameElement("Everything we got, in one thing")))
		assert(Element.parseLine("Content \t= simple/ctnt.html \t\t", "=").contains(ContentElement("simple/ctnt.html")))
		assert(Element.parseLine("String-Content — <strong>SEIZE THE MEANS OF PRODUCTION!</strong>", "—").contains(
			StringContentElement("<strong>SEIZE THE MEANS OF PRODUCTION!</strong>")))
	}
}
